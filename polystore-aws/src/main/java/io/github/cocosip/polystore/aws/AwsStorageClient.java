...
/**
 * Amazon S3-backed {@link StorageClient}, targeting Amazon Web Services only. It never overrides
 * the endpoint or the addressing style; S3-compatible stores belong to the {@code s3} provider.
 *
 * <p>{@code save} buffers the stream in memory because synchronous S3 puts require a known content
 * length. When {@code createContainerIfNotExists} is enabled the bucket is created lazily right
 * before the first upload, exactly like the reference provider does. {@code getUrl} returns a
...
public final class AwsStorageClient implements StorageClient {
     ...
     * Creates the client.
     *
     * @param client                      initialized S3 client for the AWS region, never
     *                                    {@code null}
     * @param presigner                   initialized presigner matching the client configuration,
     *                                    never {@code null}
     * @param containerName               target bucket, never {@code null}
    ...
    public void save(String fileName, InputStream inputStream, SaveArgs args) {
        ...
        try {
            PutObjectRequest.Builder builder =
                    PutObjectRequest.builder().bucket(containerName).key(fileName);
    ...
    private void createContainer() {
        try {
            ...
            throw new StorageOperationException("Failed to create bucket: " + containerName, e);
        }
    }
}
